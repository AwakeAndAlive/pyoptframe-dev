...
# Variável para capturar a primeira melhoria
first_improvement = None

class CaptureFirstImprovement:
    def __init__(self):
        self.first_improvement = None

    def callback(self, iteration, temperature, current_solution, current_evaluation, best_solution, best_evaluation, best_iteration, best_temperature):
        if self.first_improvement is None and best_evaluation < initial_evaluation:
            self.first_improvement = best_evaluation

capture_first_improvement = CaptureFirstImprovement()

start_time = time.time()

# Adicionando o callback para capturar a primeira melhoria
...
# Calcular a melhoria em relação à solução inicial
improvement = 100.0 * (initial_evaluation - best_evaluation) / initial_evaluation

# Imprimir resultados no formato CSV
print(f"{instance_name};{initial_evaluation};{best_evaluation};{improvement:.2f};{execution_time:.2f};{best_solution.cities}")
